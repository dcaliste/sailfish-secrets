                                                 ...
                                                 const QString &home)
{
    ...
    if (gpgProcess.exitStatus() != QProcess::NormalExit) {
        ...
        case QProcess::FailedToStart:
            return Result(Result::CryptoPluginKeyImportError,
                          QStringLiteral("Cannot fetch key %1 from %2: failed to start.").arg(fingerprint).arg(url));
        case QProcess::Crashed:
            return Result(Result::CryptoPluginKeyImportError,
                          QStringLiteral("Cannot fetch key %1 from %2: crashed.").arg(fingerprint).arg(url));
        case QProcess::Timedout:
            return Result(Result::CryptoPluginKeyImportError,
                          QStringLiteral("Cannot fetch key %1 from %2: timed out.").arg(fingerprint).arg(url));
        default:
            return Result(Result::CryptoPluginKeyImportError,
                          QStringLiteral("Cannot fetch key %1 from %2.").arg(fingerprint).arg(url));
        }
    }
    ...
    if (!gFetchedKey) {
        return Result(Result::CryptoPluginKeyImportError,
                      QStringLiteral("Cannot fetch key %1 from %2: %3.").arg(fingerprint).arg(url).arg(gFetchedKey.error()));
    }
                                               ...
                                               const QString &home)
{
    ...
    if (customParameters.contains("url")) {
        return downloadKey(data, customParameters.value("url").toString().toUtf8(),
                           importedKey, home);
    }
